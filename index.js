    ...
    try {
        ...
        if (username && email && password) {
            ...
            if (finduser) {
                ...
                if (datasave) {
                    req.session.user = datasave.name
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (email && password) {
            const fetcheadminmail = await adminModel.findOne({ adminEmail: email })
            if (fetcheadminmail) {
                if (fetcheadminmail.adminPassword === password) {
                    return res.render('bookok', { message: "login successfully" })
                }else {
    ...
    try {
        ...
        if (name && email && password && role) {
            ...
            if (finduser) {
                ...
                if (datasave) {
                    return res.render('bookok', { message: 'Admin Register is Successfully' })
                } else {
...
app.get('/demo',async(req,res)=>{
    try{
        const books = await adminModel.find({})
        console.log(books)
        return res.send({success:true,message:books })
    } catch (err) {
        res.status(500).send("Error  book");
    }
})
